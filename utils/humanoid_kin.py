...
class Joint(object):
  def __init__(self, info):
    ...
    if self._type is not JointType.FIXED:
...
class HumanoidSkeleton(object):
  ...
  def _calc_ctrl_info(self, ctrl_file):
    ...
    for i in range(self.num_joints):
      if self.joint_types[i] is not JointType.FIXED:
        ...
        self.torque_lim += [self.joints[i]._torque_lim] * self.joint_dof[i]
  ...
  def get_joint_pos(self, i):
    """ Return joint position
      ...
      *NOTICE* set_pose should be already called
    """
    return np.array(self._kin_core.getJointPos(i))

  def get_joint_local_ori(self, i):
    """ Return joint local orientation in exponential map

      *NOTICE* set_pose should be already called
    """
    return np.array(self._kin_core.getJointLocalOri(i))
  ...
  def get_joint_vel(self, i):
    """ Return joint position
      ...
      *NOTICE* set_pose should be already called
    """
    return np.array(self._kin_core.getJointVel(i))

  def get_joint_local_omg(self, i):
    """ Return joint local omega

      *NOTICE* set_pose should be already called
    """
    return np.array(self._kin_core.getJointLocalOmg(i))

  def get_body_pos(self, i):
    """ Return body position

      *NOTICE* set_pose should be already called
    """
    return np.array(self._kin_core.getBodyPos(i))

  def get_body_vel(self, i):
    """ Return body position

      *NOTICE* set_pose should be already called
    """
    return np.array(self._kin_core.getBodyVel(i))
  ...
  def get_pos_for_gram(self):
    """ Return 63 dim pos vector for [Holden et al. 2016]

      *NOTICE* set_pose and set_vel should be already called
    """
    joint_ids = [0, 13, 14, 15, 16, 5, 6, 7, 8, 1, 2, 3, 4, 17, 18, 19, 20, 9, 10, 11, 12]
    return np.array(self._kin_core.getJointGlobalPoses(joint_ids))
  ...
  def get_state_diff_dim(self):
    dim = 3 + self.num_joints + len(self.end_effectors)
    return dim

  def set_state_diff_lim(self, lim):
    dim = 3 + self.num_joints + len(self.end_effectors)
    assert(lim.size == dim and "limitation dim not match")
    self._kin_core.setStateDiffLim(lim)

  def calc_state_diff(self, pose0, pose1, rel_root_pos=True, rel_root_ori=True, rel_endeffector=True):
    """ Calculate difference vector between pose0 and pose1

      Inputs:
        pose0       np.array of float, pose of character
        pose1       np.array of float, pose of character
        rel         bool, calculate root pos and root orientation difference
                      in local/relative coordinate ( x,z is 0 and rotate to heading
                      direction)

      Outputs:
        state_diff  np.array of float, state difference
                    - root pos diff
                    - root orientation quaternion difference
                    - joints quaternion difference
                    - end_effectors pos diff in root's local frame
    """
    assert(len(pose0) == self.dof), "pose 0 size missmatch"
    assert(len(pose1) == self.dof), "pose 1 size missmatch"
    return np.array(self._kin_core.calcStateDiff(pose0, pose1, rel_root_pos, rel_root_ori, rel_endeffector))

  def check_state_diff(self, pose0, pose1, rel_root_pos=True, rel_root_ori=True, rel_endeffector=True):
    """ Check if state diff is out of bound
    """
    assert(len(pose0) == self.dof), "pose 0 size missmatch"
    assert(len(pose1) == self.dof), "pose 1 size missmatch"
    return self._kin_core.checkStateDiff(pose0, pose1, rel_root_pos, rel_root_ori, rel_endeffector)

  def check_state_diff_vec(self, pose0, pose1, rel_root_pos=True, rel_root_ori=True, rel_endeffector=True):
    """ Check if state diff is out of bound for each channel
    """
    assert(len(pose0) == self.dof), "pose 0 size missmatch"
    assert(len(pose1) == self.dof), "pose 1 size missmatch"
    return self._kin_core.checkStateDiffVec(pose0, pose1, rel_root_pos, rel_root_ori, rel_endeffector)
  ...
  def disturb_root_vel(self, vel, vel_noise, ang_noise):
    """ uniform disturb pose root
    """
    new_vel = vel.copy()

    noise3d = vel_noise / np.sqrt(3)
    new_vel[:3] += uniform(-noise3d, noise3d, 3)


    noise3d = ang_noise / np.sqrt(3)
    new_vel[3:6] += uniform(-noise3d, noise3d, 3)
...
if __name__=="__main__":
  ...
  mocap_file = "/home/likem/Work/motionBound/data/motions/humanoid3d_run.txt"

  # test HumanoidSkeletion
  kin_char = HumanoidSkeleton(char_file, ctrl_file)
  assert(kin_char.num_joints == 15)
  assert(kin_char.dof == 43)
  print("init pass")

  from humanoid_mocap import HumanoidMocap
  mocap = HumanoidMocap(kin_char, mocap_file)
  data = dict()
  data["pose"] = []
  data["vel"] = []
  for i in np.arange(0, 1, 0.01):
    t = i * mocap._cycletime
    cnt, pose, vel = mocap.slerp(t)
    data["pose"].append(pose)
    data["vel"].append(vel)

  # test set_pose, set_vel, build_state TODO
  for pose, vel in zip(data["pose"], data["vel"]):
    kin_char.set_pose(pose)
    kin_char.set_vel(vel)
  print("set_pose pass")
  print("set_vel pass")
  print("build_state pass")

  # test comp2pad
  for vel in data["vel"]:
    new_vel = vel[kin_char.comp2pad]
    assert(sum(new_vel) == sum(vel))
  print("comp2pad pass")

  # test slerp TODO

  # test computeVel TODO

  # test action_to_targ_pose TODO
  # test targ_pose_to_action TODO
  for pose in data["pose"]:
    action = kin_char.targ_pose_to_action(pose)
    pose_rev = kin_char.action_to_targ_pose(action)

    diff = pose[7:] - pose_rev[7:]
    assert(np.abs(diff).max() < 1e-5), embed()
  print("action_to_targ_pose pass")
  print("targ_pose_to_action pass")

  # test action_as_offset TODO

  # test exp_to_action TODO
  # test targ_pose_to_exp
  for pose in data["pose"]:
    action = kin_char.targ_pose_to_action(pose)
    exp = kin_char.targ_pose_to_exp(pose)
    new_action = kin_char.exp_to_action(exp)

    diff = action - new_action
    assert(np.abs(diff).max() < 1e-6), embed()
  print("targ_pose_to_exp pass")
  print("exp_to_action pass")

  # test a bound
  a_min = kin_char.build_a_min()
  a_max = kin_char.build_a_max()
  print("a_min and a_max pass")

  # test disturb
  for pose, vel in zip(data["pose"], data["vel"]):
    kin_char.disturb_pose(pose, 0.3)
    kin_char.disturb_vel(vel, 0.3)
  print("disturb pose pass")
  print("disturb vel pass")
