 ...
 *    - joint rots theta or (w, x, y, z) in local frame
 *
 *  vel representation:
 *    - root vel (vx, vy, vz)
...
public:
  ...
  const dVec getJointGlobalPoses(const iVec& ids) const;
...
protected:
  ...
  std::vector<dQuat> _joint_attach_quat;
  dMat _joint_revolute_axis;
