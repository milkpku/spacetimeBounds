...
Eigen::Vector3d CharDesc::getJointAttach(int id) const
{
  assert(id < this->joints.size());
  const JointDesc &j = joints[id];
  return Eigen::Vector3d(j.AttachX, j.AttachY, j.AttachZ);
}


Eigen::Vector3d CharDesc::getJointRevoluteAxis(int id) const
{
  assert(id < this->joints.size());
  const JointDesc& j = joints[id];
  return Eigen::Vector3d(j.AxisX, j.AxisY, j.AxisZ);
}

Eigen::Quaterniond CharDesc::getJointThetaAttach(int id) const
{
  assert(id < this->joints.size());
  const JointDesc& j = joints[id];
  Eigen::AngleAxisd roll(j.AttachThetaX, Eigen::Vector3d::UnitX());
  Eigen::AngleAxisd pitch(j.AttachThetaY, Eigen::Vector3d::UnitY());
  Eigen::AngleAxisd yaw(j.AttachThetaZ, Eigen::Vector3d::UnitZ());
  Eigen::Quaterniond attach = roll * pitch * yaw;

  return attach;
}

Eigen::Vector3d CharDesc::getBodyAttach(int id) const
{
  assert(id < this->bodys.size());
  const BodyDesc &b = bodys[id];
...
void from_json(const json& j, JointDesc& p) {
  ...
  if (j.find("AxisX") != j.end()) {
    j.at("AxisX").get_to(p.AxisX);
    j.at("AxisY").get_to(p.AxisY);
    j.at("AxisZ").get_to(p.AxisZ);
  }
  else
  {
    // default Z axis
