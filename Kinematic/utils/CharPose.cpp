...
bool CharPose::initJoints(const CharDesc& data, double scale)
{
  ...
  _joint_attach_quat.assign(_num_joints, dQuat::Identity());
  _joint_revolute_axis.resize(3, _num_joints);
  for (int i = 0; i < _num_joints; i++)
  {
    _joint_local_pos.col(i) = data.getJointAttach(i) * scale;

    _joint_attach_quat[i] = data.getJointThetaAttach(i);

    _joint_revolute_axis.col(i) = data.getJointRevoluteAxis(i);
  }
...
const dVec CharPose::getJointGlobalPoses(const iVec& ids) const
{
  // setPose() must be called before
  dVec poses = dVec::Zero(ids.size()*3);
  for (int i = 0; i < ids.size(); i++)
  {
    assert(ids(i) < _num_joints && ids(i) >= 0 && "ids out of range");
    poses.segment(i*3, 3) = _joint_global_pos.col(ids(i));
  }
  return poses;
}
...
{
  int dim = 3 + _num_joints + _end_effector.size();
  return dim;
}
    ...
    bool rel_root_pos, bool rel_root_ori, bool rel_endeffector)
{
  ...
  // com difference
  dVec3 com_diff = (com_pos0 - com_pos1).cwiseAbs();
  ...
  state_diff.segment(offset, 3) = com_diff;
  offset += 3;
