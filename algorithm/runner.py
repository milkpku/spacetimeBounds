...
def deepmimic_runner(filename, num_env, nsamples):
  from subproc_vec_env import SubprocVecEnv
  from model import Normalizer, Actor, Critic
  from deepmimic_sim import DeepMimicSimEnv
  import gym.spaces

  # create deepmimic env
  def make_env():
    return lambda: DeepMimicSimEnv("jump")

  vec_env = SubprocVecEnv([make_env() for i in range(num_env)])

  # load deepmimic model
  env =  DeepMimicSimEnv('jump')

  s_dim = 197
  a_dim = 36
  #ac_low = np.array(env._core.BuildActionBoundMin(0))
  #ac_high = np.array(env._core.BuildActionBoundMax(0))
  ac_low = -np.ones(a_dim)
  ac_high = np.ones(a_dim)
  ac_space = gym.spaces.Box(ac_low, ac_high)
  val_min = 0
  val_max = 20

  s_norm = Normalizer(s_dim, -1)
  actor = Actor(s_dim, a_dim, ac_space, [1024, 512])
  critic = Critic(s_dim, val_min, val_max, [1024, 512])
  ...
  try:
    ...
    print("load network succeed")
  except:
    print("Fail to load")
    assert(False)

  # create runner
  runner = RunnerGAE(vec_env, s_norm, actor, critic, nsamples, 0.95, 0.95, 1.0)

  return runner

def test_cpu():
  # test the ability to sample according to env and model
  import time
  env_nums = [1, 2, 4, 8, 12, 16]
  ts = []
  vs = []
  for i in env_nums:
    nsamples = 4096
    runner = deepmimic_runner("./torch_policy/jump.tar", i, nsamples)
  ...
  for i, v in zip(env_nums, vs):
    print("test env_num %d, sample speed %f fps" % (i, v))

def test_run_test():
  # test the ability to sample according to env and model
  env_nums = 12
  nsamples = 4096
  runner = deepmimic_runner("./torch_policy/jump.tar", env_nums, nsamples)
  from IPython import embed; embed()
  avg_step, avg_rwd = runner.test()

  print("test result: avg_step %f, avg_rwd %f" % (avg_step, avg_rwd))
