...
class Normalizer_FDM(nn.Module):
  def __init__(self, in_dim, non_norm, sample_lim=NORM_SAMPLES):
    super(Normalizer_FDM, self).__init__()
  ...
  def update(self):
    ...
    # NOTICE hack method
    # only the first two representing phase and phase velicty should not be normalized
...
def load_FDM(ckpt):
  ...
  s_norm = Normalizer_FDM(s_dim, non_norm=[], sample_lim=-1) # TODO auto config non_norm
  actor = Actor_FDM(s_dim, a_dim, a_min, a_max, a_noise, mem_size, a_hidden)
...
def unittest():
  ...
  # Test Normalizer
  s_norm = Normalizer_FDM(STATE_DIM, NON_NORM, 10000)
  ...
  s_norm = Normalizer_FDM(STATE_DIM, NON_NORM, 100)
  offset = 0
  for epoch in seq:
    for i in range(epoch):
      s_norm.record(tensor[offset])
  ...
  ## test batch record
  s_norm_dummy = Normalizer_FDM(STATE_DIM, NON_NORM, 10000)
  data = np.random.rand(TEST_NUM, STATE_DIM)
  mean = np.mean(data, axis=0)
  std = np.std(data, axis=0) + 0.01
  for i in NON_NORM:
  ...
  actor = Actor_FDM(STATE_DIM, ACTION_DIM, ACTION_STD, MEM_SIZE)
...
if __name__=="__main__":
  from IPython import embed
  #load_FDM("./torch_policy/run-alien-FDM2-2019-Apr-04-175506_15500.tar")
  unittest()
